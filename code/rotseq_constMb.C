...
int main(){
  ...
  using namespace Unites ;
	...
	delta_mer_kep, mer_mass, mermax_poisson, graph, nz, nzet, nzadapt, flaga, flagb,
	nt, np, mer_triax, n_conf, diffrot_i ;
    double entc_min, entc_min_n, entc_max, entc_max_n, fact_omega, mbar_wanted, precis, freq_min, freq_max, freq_min_si,
	    freq_max_si, thres_adapt, aexp_mass, relax, relax_poisson,
	    ampli_triax, precis_adapt, rrot_km, arot, bestpar[13] ;  

ofstream fichresu("seq.d") ;
fichresu << "# M/R  M [poly]  M_B [poly]  J [G M_sol^2/c] M [M_sol]   f_c [Hz]     H_c     r_p/r_e"
             << "       T/W       M_B [M_sol]    GRV2       GRV3    rho_c" << endl ;
    
for (entc_max = 0.26; entc_max < 1.0; entc_max+=0.05)
{   
	entc_min = entc_max;
	flaga = 0;
	flagb = 0;
    ...
    fich >> entc_min_n ; fich.ignore(1000,'\n') ;
    fich >> entc_max_n ; fich.ignore(1000,'\n') ;
	...
	if(abs(star.mass_b()/msol - mbar_wanted/msol) < 0.05)
	{
		if((abs(star.mass_b()/msol - mbar_wanted/msol) < abs(bestpar[9] - mbar_wanted/msol)))
		{
			bestpar[0] = ggrav * star.mass_g() / star.r_circ();
			bestpar[1] = mass_g_poly;
			bestpar[2] = mass_b_poly;
			bestpar[3] = star.angu_mom()/( qpig/(4* M_PI) * msol*msol);
			bestpar[4] = star.mass_g() / msol;
			bestpar[5] = star.get_omega_c() / (2.*M_PI) * f_unit;
			bestpar[6] = star.get_ent()()(0, 0, 0, 0);
			bestpar[7] = star.aplat();
			bestpar[8] = star.tsw();
			bestpar[9] = star.mass_b() / msol;
			bestpar[10] = star.grv2();
			bestpar[11] = star.grv3();
			bestpar[12] = star.get_ener()()(0,0,0,0);
			flaga = 1;
		}
		else
		{
			if(star.mass_b()/msol > mbar_wanted/msol)
				flagb = 1;
		}			
	}
