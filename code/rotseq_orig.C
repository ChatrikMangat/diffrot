...
int main(){
  ...
  using namespace Unites ;
	...
	double massarr[n_conf];
	double massarr2[n_conf];
	double enerarr[n_conf];
	double freqarr[n_conf];
    fich.ignore(1000,'\n') ;
    for (int l=0; l<nz; l++) {
	fich >> nr[l]; 
...
int counter = 0;
	...
	massarr[counter] = star.mass_g()/msol;
	massarr2[counter] = star.mass_b()/msol;
	enerarr[counter] = rho_c;
	freqarr[counter] = star.get_omega_c() / (2*M_PI) * f_unit;
	counter++;
	//-----------------------------------------------
    ...
    fichresu.close() ; 
    ofstream myfile ("massarr.txt");
	myfile.precision(10);
  	if (myfile.is_open())
  	{
    	for(int count = 0; count < n_conf; count ++){
        	myfile << massarr[count] << "," << enerarr[count] << "," << freqarr[count] << "," << massarr2[count] << "\n";
    	}
    myfile.close();
  	}
    // Cleaning
